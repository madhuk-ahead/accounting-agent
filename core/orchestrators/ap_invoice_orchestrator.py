...
class APInvoiceOrchestrator(AgentOrchestrator):
    ...
    async def _run_graph(
        ...
        """Invoke the compiled graph and format the response (single ainvoke; WebSocket route ~29s cap)."""
        if on_stream_message:
        ...
        # One invoke only. Streaming via astream() was slower; API Gateway WebSocket → Lambda is capped at ~29s.
